            ...
            <div className="friends flex flex-col justify-center gap-5">
              {friendGroups.map((friendGroup,group) => (
                <div
                  ...
                  key={friendGroup.id}
                >
                  <Title text={friendGroup.name} time={item*0.1+(group-1)*0.2} />
                  <div
                  ...
                  >
                    {friendGroup.friends.map((friend: friends,index) => {
                      item++;
                      if(friend.id==114) return <LinkCardForRedish key={friend.id} {...friend} time={(item-1)*0.1+group*0.2} />;
                      switch (friendGroup.type) {
                        case "large":
                            return <LinkCardLarge key={friend.id}  {...friend} />;
                        case "medium":
                          return <LinkCardMedium key={friend.id} {...friend} time={(item-1)*0.1+group*0.2} />;
                        case "small":
                          return <LinkCardSmall key={friend.id} {...friend} time={(item-1)*0.1+group*0.2} />;
                        default:
                          return null;
                      }
                    })}
