...
import { AutoResizeHeight } from "./lib/AutoResizeHeight";
import { useMaskScrollArea } from "./lib/use-mask-scrollarea";
import { clsxm } from "@/utils/helper";

import { MotionButtonBase } from "./lib/button";
...
>((props, ref) => {
  ...
  return (
    ...
    >
      {!!filename && (
        <div className="z-10 flex w-full items-center justify-between rounded-t-xl bg-cyan-400/20 px-5 py-2 text-sm">
      ...
      <div className="bg-cyan-400/5 py-4">
        <MotionButtonBase
          onClick={handleCopy}
          className={clsx(
            "absolute right-2 top-2 z-[3] flex text-xs center",
            "rounded-md border border-cyan-400/5 bg-cyan-400/80 p-1.5 text-white backdrop-blur duration-200",
        ...
        >
          <AnimatePresence mode="wait">
            {copied ? (
              <m.i
                key={"copied"}
                className="icon-[tabler--copy-check] iconify tabler--copy-check size-4"
                {...copyIconVariants}
              />
            ) : (
              <m.i
                key={"copy"}
                className="icon-[tabler--copy] size-4"
                {...copyIconVariants}
              />
            )}
          </AnimatePresence>
